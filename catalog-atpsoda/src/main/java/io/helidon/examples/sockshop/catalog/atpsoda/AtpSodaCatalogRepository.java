 ...
 * that that uses ATPSODA as a backend data store.
 */
@ApplicationScoped@Specializes@Traced
public class AtpSodaCatalogRepository extends DefaultCatalogRepository {
	...
	public AtpSodaSock getSock(String sockId) {
		...
		try {
			...
			// Find a documents in the collection.
			OracleDocument filterSpec = this.db.createDocumentFromString("{ \"id\" : \"" + sockId + "\"}");
     ...
     *
     * @return a ATPSODA DB filter for the specified tags
     */
	...
	public String createData() {
		...
		try {
			...
			for (int i = 0; i < jsonArray.size(); i++) {
				...
				OracleDocument doc = this.db.createDocumentFromString(jsonArray.get(i).toString());
